        ...
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (emailAddress) {
                    ...
                    yield (0, config_1.sendEmail)(emailAddress, url, 'Xác nhận email của bạn');
                }
                res.status(enums_1.StatusCode.SUCCESS).json({
                    results: enums_1.Results.SUCCESS,
                    status: enums_1.StatusText.SUCCESS,
    ...
    googleLogin(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (user && user.type !== enums_1.TypeAuth.GOOGLE) {
                    res.status(enums_1.StatusCode.ERROR).json({
                        results: enums_1.Results.ERROR,
                        status: enums_1.StatusText.ERROR,
                        message: 'You has been register this email by register email or facebook',
                    });
                    return;
                }
                if (user) {
            ...
            `;
                ...
                if (user && user.type !== 'facebook') {
                    res.status(enums_1.StatusCode.ERROR).json({
                        results: enums_1.Results.ERROR,
                        status: enums_1.StatusText.ERROR,
                        message: 'You has been register this email by register email or google',
                    });
                    return;
                }
                if (user) {
    ...
    resetPassword(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const newUser = yield services_1.userService.updateAll(user.id, user);
                res.status(enums_1.StatusCode.SUCCESS).json({
                    results: enums_1.Results.SUCCESS,
                    status: enums_1.StatusText.SUCCESS,
                    data: {
    ...
    confirmEmail(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const accessToken = services_1.authService.signForgotPassword(user.id);
                const token = encodeURIComponent(accessToken).replaceAll('.', '_');
                const url = `${process.env.BASE_URL}/forgot-password/${token}`;
                yield (0, config_1.sendEmail)(emailAddress, url, 'Quên mật khẩu', 'Xác nhận email thành công. Vui lòng nhấn nút hoặc nhấn vào đường link để cài lại mật khẩu');
                res.status(enums_1.StatusCode.SUCCESS).json({
                    results: enums_1.Results.SUCCESS,
                    status: enums_1.StatusText.SUCCESS,
                    data: {
    ...
    forgotPassword(req, res) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                ...
                const token = bearer.split('.')[1];
                const decode = services_1.authService.verifyTokenBuffer(token);
                const user = yield services_1.userService.getByIdHashTag(Number(decode.id));
                if (!user) {
                    res.status(enums_1.StatusCode.NOT_FOUND).json({
                        results: enums_1.Results.ERROR,
                        status: enums_1.StatusText.FAILED,
